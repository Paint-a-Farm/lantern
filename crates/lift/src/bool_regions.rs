...
pub fn detect_and_or_ternaries(instructions: &[Instruction]) -> (Vec<AndOrTernary>, FxHashSet<usize>) {
    let ternaries = find_and_or_ternaries(instructions);
    let mut suppressed = FxHashSet::default();

    for t in &ternaries {
        // Suppress both jumps and the fallback to prevent block boundaries
...
fn find_and_or_ternaries(instructions: &[Instruction]) -> Vec<AndOrTernary> {
    ...
    while pc < instructions.len() {
        ...
        if insn.op == OpCode::JumpIfNot {
            ...
            // Scan forward for a JumpIf Rb that targets fallback_pc + K (past the fallback)
            // The JumpIf must be BEFORE fallback_pc and use a potentially different register.
            let mut scan = pc + 1;
            while scan < fallback_pc && scan < instructions.len() {
                ...
                if scan_insn.op == OpCode::JumpIf {
                    ...
                    if join_pc > fallback_pc
                        ...
                        // The JumpIf should be the last jump before fallback
                        && scan + 1 <= fallback_pc
                    {
...
/// Check if instructions between `from` and `to` have side effects beyond simple loads.
fn tail_has_side_effects(instructions: &[Instruction], from: usize, to: usize) -> bool {
    for pc in from..to {
        if pc >= instructions.len() { return true; }
        let insn = &instructions[pc];
...
pub fn detect_value_ternaries(instructions: &[Instruction]) -> (Vec<ValueTernary>, FxHashSet<usize>) {
...
fn find_value_ternaries(instructions: &[Instruction]) -> Vec<ValueTernary> {
    ...
    for pc in 0..instructions.len() {
        ...
        // between false_val_pc and join_pc
        ...
        // (the instruction just before the Jump).
        ...
        // Verify: cond_reg != result_reg (otherwise it's an or-chain, not a ternary)
        // Actually this CAN happen for `x = x and "a" or "b"` but that's fine to handle.
        // The key distinguisher: we need the Jump to truly skip the false value.
        // If join_pc == false_val_pc, there's no false branch — skip.
